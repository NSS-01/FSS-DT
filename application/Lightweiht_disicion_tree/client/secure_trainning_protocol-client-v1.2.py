...
from application.Lightweiht_disicion_tree.config_tree import m, m0, client_data, unique_values_by_column
...
g = ArithmeticSharedRingTensor(torch.tensor(2 ** (BIT_LEN - 1) - 1, dtype=torch.int64), party=party)
nk = ArithmeticSharedRingTensor(torch.tensor(0, dtype=torch.int64, device=DEVICE), party=party)
ny = ArithmeticSharedRingTensor(binary_matrix*party.party_id,party=party)


def SecureBuildTree(data, share_w: ArithmeticSharedRingTensor, T: list, h: int,Maximum_depth:int):
    if h in range(0, 2 ** Maximum_depth - 1):
        ...
        T[h].t = z * t - (1 - z.int())
        b_ready = (data[:, m1.int()] > t) * share_w.party.party_id
        b = ArithmeticSharedRingTensor(b_ready, dtype=torch.int64, party=share_w.party)
        w2h = share_w * b
    ...
    elif h in range(2 ** Maximum_depth - 1, 2 ** (Maximum_depth + 1) - 1):
        y_sum = (share_w.view(n_samples, -1) * ny).sum(dim=0)
        # y_max = y_sum[0]
        # share_y = RingTensor(labels[0], dtype=int, device=DEVICE)
        # for index, y in enumerate(y_sum[1:], start=1):
        #     theta = (y - y_max) >= zero
        #     y_max = theta * y + ( - theta+RingTensor(1, dtype=int, device=DEVICE)) * y_max
        #     share_y = theta * RingTensor(labels[index]) + ( - theta+RingTensor(1, dtype=int, device=DEVICE)) * share_y
...
def SecureFindBestSplit(data, share_w: ArithmeticSharedRingTensor) -> Tuple[
    ...
    # m_= RingTensor(0, dtype=int, device=DEVICE)
    each_index_ts =[]
    each_best_gs = []
    for j in range(0, m - 1):
        ...
        # t = RingTensor(ts[0], dtype=int, device=DEVICE)
        #
        # for index, g_ in enumerate(gs[1:], start=1):
        #
        #     theta = g_ <= g
        #     g = theta * g_ + ( - theta+RingTensor(1,dtype=int,device=DEVICE)) * g
        #     t = theta *RingTensor(ts[index], dtype=torch.int64,device=DEVICE) + ( - theta+RingTensor(1,dtype=int,device=DEVICE)) * t
        #     m_ = theta * RingTensor(j, dtype=int,device=DEVICE ) + (- theta+RingTensor(1,dtype=int,device=DEVICE)) *m_
    ms = ArithmeticSharedRingTensor(torch.arange(m-1)*share_w.party.party_id,party=share_w.party)
    ts = ArithmeticSharedRingTensor.cat(each_best_gs,0)
    _,m_,t = _max_with_index_threshold(ts,ms ,ArithmeticSharedRingTensor.cat(each_index_ts,0))
    return m_, t

if __name__ == "__main__":
    # vals = ArithmeticSharedRingTensor(torch.tensor([1, 2, 2, 8, 5,5], dtype=int, device=DEVICE), party=party)
    # indices =  ArithmeticSharedRingTensor(torch.arange(0,vals.size(0))*party.party_id,dtype=int,device =DEVICE,party=party)
    # z1,z2 = _max_with_index(vals,indices,dim=0)
    # print(z1.restore())
    # print(z2.restore())

    for h  in range(3, 4, 2):
        Maximum_depth= h
        T = [TreeNode() for _ in range(0, 2 ** (Maximum_depth + 1) - 1)]
        w0 = ArithmeticSharedRingTensor(torch.zeros(n_samples, dtype=torch.int64), party=party)
        st = time()
        SecureBuildTree(client_data, share_w=w0, T=T, h=0,Maximum_depth=Maximum_depth)
        et = time()

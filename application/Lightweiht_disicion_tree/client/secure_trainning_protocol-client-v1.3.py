...
client.connect(('127.0.0.1', 20000), ('127.0.0.1', 20001), ('127.0.0.1', 8089), ('127.0.0.1', 8888))
# client.connect(('127.0.0.1', 20000), ('127.0.0.1', 20001), ('127.0.0.1', 10000), ('127.0.0.1', 10001))
party = client
zero = ArithmeticSharedRingTensor(torch.tensor(0), party=party)
g = ArithmeticSharedRingTensor(torch.tensor(2 ** (BIT_LEN - 1) - 1), party=party)
nk = ArithmeticSharedRingTensor(torch.tensor(0, device=DEVICE), party=party)
...
def SecureBuildTree(data, share_w: ArithmeticSharedRingTensor, T: list, h: int,Maximum_depth:int):
    if h in range(0, 2 ** (Maximum_depth-1) - 1):
        ...
        b = ArithmeticSharedRingTensor(b_ready,  party=share_w.party)
        w2h = share_w * b
...
if __name__ == "__main__":
    ...
    w0 = ArithmeticSharedRingTensor(torch.zeros(n_samples, dtype=torch.int), party=client)
    st = time()

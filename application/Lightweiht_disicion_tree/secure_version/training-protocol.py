...
def SecureBuildTree(m0,data, thresholds, share_w: ArithmeticSecretSharing, T: list, h: int, maxinum_depth: int, ny:ArithmeticSecretSharing):
    if h in range(0, 2 ** (maxinum_depth - 1) - 1):
        ...
        T[h].t = z * t - (1 - z.int())
        b_test = (data[:, m.int()] - t > 0) * share_w.party.party_id
        ...
        n,m = data.shape
        y_sum = (share_w.view(n, -1) * ny).sum(dim=0)
        labels = thresholds[-1].view(-1)
        share_labels = ArithmeticSecretSharing(RingTensor.convert_to_ring((-1*labels)* share_w.party.party_id), party=share_w.party)
...
def SeureFindBestSplit(m0,data, share_w, thresholds,ny):
    ...
    for j in range(0, m-1):
        ...
        wr_share = share_w.view(n,-1)-wl_share
        num_Dl = wl_share.sum(dim=0)
        num_Dr = wr_share.sum(dim=0)
        Dlk = (ny.view(-1,n))@wl_share
        Drk = (ny.view(-1,n))@wr_share

        square_Dlk = (Dlk*Drk).sum(dim=0)
        square_Drk = (Drk*Dlk).sum(dim=0)
        t = threshold.view(-1)
        ts = ArithmeticSecretSharing(RingTensor(t*share_w.party.party_id),party=share_w.party)
        gs = num_Dr*(num_Dl-square_Dlk)+num_Dl*(num_Dr-square_Drk)
        # gs =  square_Dlk/num_Dl+square_Drk/num_Dr
        '''
        choose min value of  each feature  
        '''
        g, t = _min_with_index(gs, ts)
        each_best_gs.append(g.view(-1))
        each_best_ts.append(t.view(-1))
    ms = ArithmeticSecretSharing(RingTensor.convert_to_ring(torch.arange(m - 1) * share_w.party.party_id),
                                 party=share_w.party)
...
def setup_client(data_name, max_height):
    ...
    ny = ArithmeticSecretSharing(RingTensor(torch.zeros(size=(n,num_class,),dtype=torch.int)), party=client)
    client.online()
    '''
    client training code
    '''
    root_share_w = ArithmeticSecretSharing(RingTensor(torch.zeros(n,dtype=torch.int)), party=client)
...
def setup_server(data_name, max_height):
    ...
    binary_matrix = torch.tensor((data_label == unique_label)*1,dtype=torch.int)
    server = SemiHonestCS(type='server')
    server.set_comparison_provider()
    server.set_multiplication_provider()
    ny = ArithmeticSecretSharing(RingTensor(binary_matrix),party=server)
    ...
    root_share_w = ArithmeticSecretSharing(RingTensor(torch.ones(n,dtype=torch.int)), party=server)
...
if __name__ == '__main__':
    ...
    client_thread = threading.Thread(target=setup_client, args=(data_name, max_height))
    server_thread = threading.Thread(target=setup_server, args=(data_name, max_height))
    # Starting threads
    client_thread.start()
    server_thread.start()

...
def find_best_split(data, num_features):
    ...
    for feature_idx in range(num_features):
        ...
        for threshold in thresholds:
            ...
            gini_index = DR * (DL**2 - DLK) + DL * (DR**2 - DRK)
            # g0 = DR*(-DLK)+DL*(-DRK)
...
# Split the hashed dataset into a training set and a test set
...
clf = DecisionTreeClassifier(max_depth=5, criterion="log_loss")
...
clf = DecisionTreeClassifier(max_depth=10, criterion="log_loss")
...
clf = DecisionTreeClassifier(max_depth=15, criterion="log_loss")

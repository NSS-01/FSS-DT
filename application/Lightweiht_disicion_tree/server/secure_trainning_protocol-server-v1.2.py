...
def SecureFindBestSplit(data, share_w: ArithmeticSharedRingTensor) -> Tuple[
    ...
    for j in range(0, m - 1):
        ...
        #     t = theta *RingTensor(ts[index], dtype=int,device=DEVICE) + ( - theta+RingTensor(1,dtype=int,device=DEVICE)) * t
...
if __name__ == "__main__":
    ...
    # vals= ArithmeticSharedRingTensor(torch.tensor([1,2,2,7,5,17],dtype=int,device= DEVICE), party =party)
    # indices = ArithmeticSharedRingTensor(torch.arange(vals.size(0))*party.party_id, dtype=int, device=DEVICE, party=party)
    ...
    for h  in range(3, 4):
        ...
        w1 = ArithmeticSharedRingTensor(torch.ones(n_samples, dtype=torch.int64), party=party)
        st = time.time()
        SecureBuildTree(server_data, share_w=w1, T=T, h=0,Maximum_depth=Maximum_depth)
        et = time.time()
        print(f"h: {h+1}, running time:{et-st}")
        log.append(f"h: {h+1}, running time:{et-st}")

...
def inference(x, tree_list, tree_indices, p,c, h):
    ...
    p0 =ArithmeticSharedRingTensor(torch.tensor(zore,dtype=torch.int),party=party)
    p1 = ArithmeticSharedRingTensor(torch.tensor(p,dtype=torch.int), party=party)
    res = (p0*p1*label).sum()
    return res
...
if __name__ == "__main__":
    h = 4
    tree_indices = [x for x in range(0, 2 ** h - 1)]
    T = [Tree(f=-1,t=1) for x in range(0, 2 ** h - 1)]

...
class TestRssMalicious(unittest.TestCase):
    ...
    def test_04_mat_mul(self):
        for i in range(10):
            ...
            _ = x @ y
            end_time = time.time()
            print("normal v mat mul time: ", end_time - start_time)


    def test_05_mat_mul_with_trunc(self):
        for i in range(10):
            start_time = time.time()
            _ = v_matmul_with_trunc(x, y)
            end_time = time.time()
            print("v mat mul time with trunc: ", end_time - start_time)

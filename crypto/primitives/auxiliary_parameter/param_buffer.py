...
from config.base_configs import DEBUG, DEVICE, base_path
...
class ParamBuffer(object):
    ...
    def load_param(self):
        self.param = self.param_type.load_from_file_pipe(self.party.party_id, self.num_of_party)
        self.load_buffer = self.param.clone()
        self.left_ptr = len(self.param)

    def load_keys_by_name(self, name, file_path):  # TODO 能否去掉
        dic = ParamFilePipe.read_dic_by_name(name, file_path)
        self.param = self.param_type.from_dic(dic)
    ...
    def load_buffers(self):
        ...
        with self.lock:
            while num_of_params > 0 and os.path.exists(param_path + file_name):
                dic = ParamFilePipe.read_dic_by_name(file_name, param_path)
                buffer_params = self.param_type.from_dic(dic)
    ...
    def get_parameters(self, number_of_params):
        ...
        if DEBUG:
            param = self.param[0].to(DEVICE)  # TODO 不要广播
            return param

...
class PosExp(object):
    ...
    def eval(x: ArithmeticSharedRingTensor, key, scale_bit=16):
        return pos_exp_eval(x, key, scale_bit)


class PosExpKey(Parameter):
    def __init__(self):
    ...
    def __getitem__(self, item):
        key = super(PosExpKey, self).__getitem__(item)
    ...
    def gen(num_of_keys, scale_bit=16):
        return pos_exp_gen(num_of_keys, scale_bit)


def pos_exp_gen(num_of_keys, scale_bit=16):
    each_table_bit = (scale_bit + 6) // 2

    k0, k1 = PosExpKey(), PosExpKey()
...
def pos_exp_eval(x, key, scale_bit=16):
    total_bit = scale_bit + 6
    shape = x.shape

    high_table = key.high_table
    low_table = key.low_table

    lt = x < ArithmeticSharedRingTensor(RingTensor(2 ** (total_bit - 1), dtype=x.dtype, device=x.device), x.party)

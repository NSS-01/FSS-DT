...
from NssMPC.config import BIT_LEN, HALF_RING
...
def secure_ge(x, y):  # TODO mac
    ...
    out_pre, out_next = equal_match_all_dpf(ArithmeticSecretSharing(RingTensor(0), pre_party), input_pre, input_next,
                                            ...
                                            key_from_previous, key_from_next, 8)
    ...
    res.dtype = dtype
    return res.reshape(shape) * res.scale
...
def equal_match_all_dpf(xp, yp, xn, yn, kp, kn, bit_len):
    ...
    res2_p = res2_p.view(res1_p_shift.size()).sum(-1)

    res2_n = DPF.eval(res1_n_shift.view(-1, len(kn1), 1), kn1, party_n.party_id)
    res2_n = res2_n.view(res1_n_shift.size()).sum(-1)
    ...
    res_p = DPF.eval(res2_p_shift.view(-1, 1), kp2, party_p.party_id)
    res_n = DPF.eval(res2_n_shift.view(-1, 1), kn2, party_n.party_id)

from crypto.protocols.exp.pos_exp import *
...
# normal_x = torch.tensor([[0.01], [1.2], [1.1], [65]], device=DEVICE)
normal_x = torch.arange(21, 22, step=0.1, device=DEVICE)
...
res = pos_exp_eval(x0, k0)

print(res.restore().convert_to_real_field().to(torch.int64))

print(torch.exp(normal_x).to(torch.int64))
